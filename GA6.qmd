...
3.  In the above plot change the theme to `theme_bw` and color the `state` manually instead of using default colors. Add the title of the plot using the **code chunk options**. Save this plot as an object `violin_plot`.
    ...
    ```{r violin_plot, warning=FALSE, fig.cap="Population by State and Category"}
    state_colors <- c(
      IL = "purple",
      IN = "green",
      MI = "orange",
...
4.  Export the `violin_plot` and label it as `violoin_plot.png` in your current working repository.
    ...
    ```{r violin_plot_saved, warning=FALSE}
    ggsave(
...
Or I could do the same manually by running the plot and exporting the output as a .png file using "export" button.

**Publish your repo on Github**:

```         
git add GA6.qmd violoin_plot.png GA6.html GA6_files
git commit -m "final commit"
git branch -M main
git remote add origin git@github.com:kolganovaanna/GA6.git
